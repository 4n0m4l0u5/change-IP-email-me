...
#sudo apt-get install postfix mailutils libsasl2-2 ca-certificates libsasl2-modules curl
#choose "internet" (SMTP) mail service and use default FQDN

#for gmail (or if you have something other than gmail you have to figure out what they use), add these lines to /etc/postfix/main.cf (remove hashtags)... eg. sudo nano /etc/postfix/main.cf also, if you have two step verification, you will have to generate an app password in gmail and use it in the next section (by the way, this is not very secure to keep your password in a file on your computer, even with no read permissions):
...
#open/create file: /etc/postfix/sasl_passwd	eg. sudo nano /etc/postfix/sasl_passwd
#enter your info like so (no hashtag and use your username and app password):
#[smtp.gmail.com]:587    USERNAME@gmail.com:PASSWORD
#save and change file permissions with this code:
#sudo chmod 400 /etc/postfix/sasl_passwd
#sudo postmap /etc/postfix/sasl_passwd

#validate certificates by running this code:
#cat /etc/ssl/certs/Thawte_Premium_Server_CA.pem | sudo tee -a /etc/postfix/cacert.pem
#you may need to look these certificates up and install them manually if your disro does not come with them pre installed (linux mint 18 or ubuntu 16.04 for example)
